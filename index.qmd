...
# Create the ggplot
plot_3 <- ggplot(filtered_data, aes(x = Year, y = prevalence / 1e6, color = age_group, group = age_group)) +
  geom_line() +
  geom_point() +
  scale_x_continuous(breaks = seq(min(filtered_data$Year), max(filtered_data$Year), by = 5)) +
  scale_y_continuous(labels = label_number(suffix = "M")) +
  labs(title = "Trends in Neoplasm Prevalence by Age Group around the world",
       x = "Year",
       y = "Number of People with Neoplasms") +
  theme_minimal() +
...
# #   select("Deaths - Neoplasms - Sex: Both - Age: Under 5 (Rate)",

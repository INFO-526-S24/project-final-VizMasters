...
# Merge world map data with filtered data
world_data <- merge(world, filtered_data, by.x = "name_long", by.y = "Entity")

# Function to create continent-specific maps
create_continent_map <- function(continent_name) {
  continent_data <- world_data %>%
    filter(continent == continent_name)
  
  p <- ggplot() +
    geom_sf(data = continent_data, aes(fill = Deaths_Neoplasms), color = "black", size = 0.5) +
    scale_fill_gradient(low = "#fff7bc", high = "#cc4c02", name = "Neoplasm Deaths") +
    labs(title = paste("Share of population with cancer per 100 people -", continent_name, "- 2000")) +
    theme_void() +
...
continent_names <- unique(world_data$continent)

# Create dropdown menu
buttons <- list(
  list(method = "update",
       args = list(list(visible = list(world_data$continent %in% continent_names))),
       label = "World")
)

for (i in 1:length(continent_names)) {
  buttons[[i + 1]] <- list(method = "update",
                           args = list(list(visible = list(world_data$continent == continent_names[i]))),
                           label = continent_names[i])
}
...
continent_plot <- ggplotly(create_continent_map("Asia")) %>%
  layout(updatemenus = updatemenus)

# Show the continent plot
continent_plot
...
# Create a color palette for continents
continent_colors <- c(
  "Africa" = "#1f77b4",  # Blue
  "Asia" = "#ff7f0e",     # Orange
  "Europe" = "#2ca02c",   # Green
...
# Plot
p <- ggplot() +
  geom_sf(data = world_data, aes(fill = continent, text = paste("Region:", name_long, "<br>Deaths:", Deaths_Neoplasms)), color = "black", size = 0.5) +
  scale_fill_manual(values = continent_colors) +
  labs(title = "Share of population with cancer per 100 people, 2000") +
  theme_void() +
  theme(legend.position = "none")
...
# Remove elementId to prevent Shiny error
p$elementId <- NULL
...
p <- ggplot() +
  ...
  scale_fill_gradient(low = "#fff7bc", high = "#cc4c02", name = "Neoplasm Deaths") +
  labs(title = "Share of population with cancer, 2000") +
  theme_void() +
  theme(legend.position = "bottom") 
